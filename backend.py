...
def setup_pipeline(pdf_path: str, chunk_size, chunk_overlap, embed_model_name="gemini-embedding-001", force_rebuild=False):
...
def chat(state:ChatStateSchema):
    ...
    prompt = ChatPromptTemplate([
        ...
        # ('human',"Answer from the provided context if context not found then try to generate it by your self. generate a detailed answer \nquestion:{question}\n context:{context}")
        ('human',"Answer from the provided context if context not found then just answer context not found. generate a detailed answer \nquestion:{question}\n context:{context}")
        ])
    chain = prompt | model
